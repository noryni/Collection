...
local old_noryn = CoreGui:FindFirstChild('Noryn')

if old_noryn then
    Debris:AddItem(old_noryn , 0)
end
...
local Config = setmetatable({
    save = function(self: any, file_name: any, config: any)
        local success_save, result = pcall(function()
            ...
            writefile('Noryn/'..file_name..'.json', flags)
        end)
    ...
    load = function(self: any, file_name: any, config: any)
        local success_load, result = pcall(function()
            if not isfile('Noryn/'..file_name..'.json') then
                self:save(file_name, config)
        
                return
            end
        
            local flags = readfile('Noryn/'..file_name..'.json')
...
function Library:create_ui()
    ...
    local Noryn= Instance.new('ScreenGui')
    Noryn.ResetOnSpawn = false
    Noryn.Name = 'Noryn'
    Noryn.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    ...
    function self:UIVisiblity()
        Noryn.Enabled = not Noryn.Enabled;
    end;
    ...
    function self:load()
        ...
        for _, object in Noryn:GetDescendants() do
    ...
    function self:create_tab(title: string, icon: string)
        ...
        function TabManager:create_module(settings: any)
            ...
            if not settings.rich then
                ...
                ModuleName.Text = settings.richtext or "<font color='rgb(255,0,0)'>Noryn</font> user"
            end;
            ...
            function ModuleManager:create_paragraph(settings: any)
                ...
                if not settings.rich then
                    ...
                    Body.RichText = true
                    Body.Text = settings.richtext or "<font color='rgb(255,0,0)'>Noryn</font> user"
                end
            ...
            function ModuleManager:create_text(settings: any)
                ...
                function TextManager:Set(new_settings)
                    if not new_settings.rich then
                        ...
                        Body.RichText = true
                        Body.Text = new_settings.richtext or "<font color='rgb(255,0,0)'>Noryn</font> user"
                    end
                end;
